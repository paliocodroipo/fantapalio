...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Funzione per aggiornare la selezione del giocatore nella tendina
    const updatePlayerSelection = (playerName) => {
        ...
        // Trigger il change event per aggiornare le statistiche del giocatore
        const event = new Event('change');
        select.dispatchEvent(event);
    };

    // Ottieni il nome del giocatore dalla query string
    const urlParams = new URLSearchParams(window.location.search);
    const playerName = urlParams.get('playerName');

    // Se il nome del giocatore è presente, aggiorna la selezione
    if (playerName) {
        updatePlayerSelection(decodeURIComponent(playerName));
    } else {
        // Se nessun giocatore è selezionato, mostra il primo giocatore della prima squadra come predefinito
        if (groupedPlayers.length > 0 && groupedPlayers[0].players.length > 0) {
            updatePlayerSelection(groupedPlayers[0].players[0].name);
        }
    }
